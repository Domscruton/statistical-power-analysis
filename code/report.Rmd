...
for (i in cDiffMeans) {
  ...
  matResults2Power[j, 2:5] <- sapply(SimulationFun(N = 20, K = 10, R = 10, 
                                              diff.means = i)$Results, 
                            "[[", 1)[2:5]
}
matResults2Power[, 1] <- cDiffMeans
# Plot results
ggplot(as.data.frame(matResults2Power)) + 
  geom_point(aes(x = N, y = Randomization.AvgPValue, 
                 col = 'Randomization Test'), size = 3) + 
  geom_point(aes(x = N, y = tTest.AvgPValue, col = 't-test'), size = 3) + 
  labs(title = 'Test Power by Effect Size', x = 'Effect Size', 
       y = 'Power', col = "Test") + 
  geom_line(aes(x = N, y = Randomization.AvgPValue, 
                col = 'Randomization Test'), size = 1.2) + 
  geom_line(aes(x = N, y = tTest.AvgPValue, col = 't-test'), size = 1.2) + 
  scale_x_continuous(trans = 'log2')

kable(matResults2Power, format = "markdown")
```
...
for (i in cVar) {
  j <- j + 1
  matResults3Power[j, 2:5] <- sapply(SimulationFun(N = 20, K = 10, R = 10, sd = i)$Results, 
                            "[[", 1)[2:5]
}
...
kable(matResults4Power, format = "markdown")
```
...
SimulationFun(N = 20, K = 10, R = 10,
      shape = unlist(matGamma[i, 1:2]), rate = unlist(matGamma[i, 3:4]), 
      distribution = "gamma")$tTestValue
```
