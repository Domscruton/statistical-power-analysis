...
As a motivating example, we also consider comparing murder rates between Southern and Northern US states; we would like to assess our a priori expectations that Southern states have higher homicide rates.
...
For large samples, the two-sample t-test is reasonably robust to deviances from normality, as attested by the __Central Limit Theorem (CLT)__.
...
Fisher suggested using _permutations_ of the data. The Data is randomly sampled into the two groups under the Null hypothesis that they have the same mean (therefore it is valid to randomly sample from the two groups to calculate an overall mean since we assume both samples are generated from a distribution with the same mean). This is done some large number $B$ times, yielding permutation t-values $t_1^*, t_2^*, ..., t_B^*$. The two-sided permutation significance level for the original value $t$ is then the proportion of the $t_i^*$ values exceeding $t$ in absolute value:
...
Provide definitions of size and power as motivation for why we are using them as measures of test performance for randomization Vs t-test.

We therefore start by assuming a somewhat unrealistic normal distribution given below:

$$X_{i}^{(S)} \sim N(0.04417, 0.1) \\ X_{i}^{(N)} \sim N(0.01848, 0.1)$$
...
A more appropriated boxplot would also appear to conclude that murder rates are lower in Northern states than Southern States. However we prefer a formal statistical test to assess our hypothesis and to quantify our uncertainty about such conclusions.
...
kable(dtMurders[, .(Obs = .N, MurderRate = mean(Rate), Variance = var(Rate)), 
                by = region], format = "markdown")
```
...
Fisherian methodology has faced criticism for its over-reliance on normal sampling assumptions. p-values are calculated based on theoretical t-dstributions, which depend on Gaussian, or Normal, assumptions. 
...
# Potentially 2-dimensional plots for comparing alpha and beta- consider the effective variance and also the mean of the gamm distribution, alpha/beta. Potentially plot the pvalue as a overlay coloured variable

#Plot each simulated gamma distribution against the South data
# hist(cSouthRate, xlab = "Murder Rate", main = "South Region Murder Rate 
...
Mention profiling of the code and how we have gone about this
