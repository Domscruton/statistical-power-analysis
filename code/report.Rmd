...
colnames(matResults1Power) <- 
  c("N", "rSizePower", "rAvgPValue", "tSizePower", "tAvgPValue")
...
for (i in cSampSize) {
  ...
  matResults1Power[j, 2:5] <- 
    sapply(SimulationFun(N = i, K = 10, R = 10, diff.means = 0.1),
           "[[", 1)[2:5]
}
...
for (i in cSampSize) {
  ...
  matResults1Size[j, 2:5] <- 
    sapply(SimulationFun(N = i, K = 10, R = 10, diff.means = 0), 
           "[[", 1)[2:5]
}
#Plot of test power against sample size
ggplot(as.data.frame(matResults1Power), aes(x = N)) + 
  geom_point(aes(y = rSizePower, col = 'Randomization Test'),
             size = 3) + 
  geom_point(aes(y = tSizePower, col = 't-test'), size = 3) + 
  labs(title = 'Test Power by Sample Size', 
       x = 'Sample Size', y = 'Power') + 
  geom_line(aes(y = rSizePower, col = 'Randomization Test'), 
            size = 1.2) + 
  geom_line(aes(y = tSizePower, col = 't-test'), size = 1.2) + 
  scale_x_continuous(trans = 'log2')
#Plot of test size against sample size
ggplot(as.data.frame(matResults1Size), aes(x = N)) + 
  geom_point(aes(y = rSizePower, col = 'Randomization Test'), size = 2) + 
  geom_point(aes(y = tSizePower, col = 'T-test'), size = 2) + 
  labs(title = 'Test Size for Increasing Sample Size', x = 'Log Sample Size', 
       y = 'Test Size') + 
  geom_line(aes(y = rSizePower, col = 'Randomization Test'), size = 1) + 
  geom_line(aes(y = tSizePower, col = 't-test'), size = 1) +
...
```{r}
kable(matResults1Power, format = "markdown")
```
...
for (i in cDiffMeans) {
  j <- j + 1
  matResults2Power[j, 2:5] <- sapply(SimulationFun(N = 20, K = 10, R = 10, 
                                              diff.means = i), 
                            "[[", 1)[2:5]
}
...
ggplot(as.data.frame(matResults2Power)) + 
  geom_point(aes(x = N, y = rAvgPValue, col = 'Randomization Test'),
             size = 3) + 
  geom_point(aes(x = N, y = tAvgPValue, col = 't-test'), size = 3) + 
  labs(title = 'Test Power by Sample Size', x = 'Sample Size', 
  y = 'Power', col = "Test") + geom_line(aes(x = N, y = rAvgPValue, 
...
for (i in cVar) {
  j <- j + 1
  matResults3Power[j, 2:5] <- sapply(SimulationFun(N = 20, K = 10, R = 10, sd = i), 
                            "[[", 1)[2:5]
}
...
```{r}
kable(matResults3Power, format = "markdown")
```
